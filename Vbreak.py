...
def read_data(filename):
    try:
        ...
        return None, None
    if data.shape[1] < 2:
        messagebox.showerror("Column Error", "The Excel file must have at least two columns.")
        return None, None
    V = data.iloc[:, 0]
    i = data.iloc[:, 1]
    mask = (i > 0) & V.notnull() & i.notnull()
    return V[mask].to_numpy(), i[mask].to_numpy()


def fit_tafel(eta_corr, i):
    """
    New Tafel fitting routine:
      1. Restrict to the current region 0.005-0.1 A/cm².
      2. Randomly select 10 candidate 60 mV windows within the available voltage range.
      3. For each candidate, define three subwindows:
           - Middle 20 mV (center ±10 mV)
           - Middle 40 mV (center ±20 mV)
           - Full 60 mV (the candidate window)
         For each subwindow, perform a linear regression of η_corr vs. log10(i)
         and compute the Tafel slope (b) and intercept (from which i₀ is computed via i₀ = 10^(-intercept/b)).
      4. For each candidate, calculate:
             - The average slope (b̄) and its standard deviation (σ_b),
             - The average i₀ (ī₀) and its standard deviation (σ_i0)
         Then compute the relative standard errors:
             SE%_b = σ_b / |b̄|
             SE%_i0 = σ_i0 / ī₀.
         Then combine these as: candidate_metric = (SE%_b + SE%_i0)/2.
      5. Choose the candidate with the smallest candidate_metric.
      6. Return the average parameters from that candidate along with all details.
    """
    # Restrict to current density range 0.005 to 0.1 A/cm².
    mask = (i >= 0.005) & (i <= 0.1)
    if np.sum(mask) < 5:
        messagebox.showerror("Data Error",
                             "Not enough data points in the current density range 0.005 to 0.1 A/cm² for Tafel fitting.")
        return None
    eta_sel = eta_corr[mask]
    i_sel = i[mask]

    # Determine overall voltage (η_corr) range.
    ...
    candidate_results = []
    # Randomly choose 10 candidate 60 mV windows.
    for candidate in range(1000):
        ...
        avg_i0 = np.mean(i0s)
        # Relative errors (SE%):
        rel_err_b = std_slope / abs(avg_slope)
        rel_err_i0 = std_i0 / avg_i0
...
class IntegratedLSVAnalysisApp:
    def __init__(self, master):
        ...
        # Define a common font for the entire window (size 24)
        self.font = ("Arial", 24)
        ...
        tk.Label(input_frame, text="Temperature (K):", font=self.font).grid(row=1, column=0, sticky="w")
        ...
        tk.Label(input_frame, text="R_CL (Ohm·cm²):", font=self.font).grid(row=2, column=0, sticky="w")
        self.rcl_entry = tk.Entry(input_frame, font=self.font, width=10)
        self.rcl_entry.insert(0, "0.1")
        self.rcl_entry.grid(row=2, column=1, padx=5, pady=5, sticky="w")

        tk.Label(input_frame, text="HFR (Ohm·cm²):", font=self.font).grid(row=3, column=0, sticky="w")
        self.hfr_entry = tk.Entry(input_frame, font=self.font, width=10)
        self.hfr_entry.insert(0, "0.01")
        self.hfr_entry.grid(row=3, column=1, padx=5, pady=5, sticky="w")

        # Note: Catalyst layer Tafel parameter is determined from the kinetic analysis.
        self.fit_button = tk.Button(input_frame, text="Fit & Plot", font=self.font,
                                    command=self.perform_fit, bg="#cce6ff")
        self.fit_button.grid(row=4, column=0, columnspan=3, pady=10)
        ...
        # Overpotential components and curves (for export)
        self.eta_kin = None
        self.Eta_ohm = None
        self.Eta_RCL = None
        self.Eta_res = None
    ...
    def perform_fit(self):
        ...
        V, i = read_data(self.filename)
        if V is None or i is None:
            return
        self.V_data = V
        self.i_data = i

        # Remove ohmic drop before Tafel fit.
        self.Eta_ohm = i * HFR
        V_corr = V - self.Eta_ohm
        eta_corr = V_corr - self.E_rev

        kinetic_fit = fit_tafel(eta_corr, i)
        if kinetic_fit is None:
            return
        b_kin = kinetic_fit["b_kin"]
        intercept = kinetic_fit["intercept"]
        i0 = kinetic_fit["i0"]
        self.eta_kin = b_kin * np.log10(i / i0)
        ...
        self.Eta_res = (V - self.E_rev) - (self.eta_kin + self.Eta_ohm + self.Eta_RCL)

        # Construct the 6 curves.
        self.y1 = np.full_like(i, self.E_rev)
        ...
        diag_info = (
            ...
            f"  Data points (0.005-0.1 A/cm²): {kinetic_fit['n_points']}\n\n"
            f"Best Candidate 60 mV Window:\n"
            f"  Voltage range: {details['window'][0]:.4f} V to {details['window'][1]:.4f} V\n"
            f"  (Center = {details['center']:.4f} V)\n"
            f"  Subwindows:\n"
            f"     20 mV: {details['subwindows']['20mV'][0]:.4f} V to {details['subwindows']['20mV'][1]:.4f} V (n={details['counts']['20mV']})\n"
            f"     40 mV: {details['subwindows']['40mV'][0]:.4f} V to {details['subwindows']['40mV'][1]:.4f} V (n={details['counts']['40mV']})\n"
            f"     60 mV: {details['subwindows']['60mV'][0]:.4f} V to {details['subwindows']['60mV'][1]:.4f} V (n={details['counts']['60mV']})\n"
            f"  Relative SE% for b: {details['rel_err_b'] * 100:.2f}%\n"
            f"  Relative SE% for i0: {details['rel_err_i0'] * 100:.2f}%\n"
            f"  Combined Metric: {details['candidate_metric'] * 100:.2f}%\n\n"
            f"Ohmic Correction:\n"
            f"  HFR: {HFR:.4f} Ohm·cm²\n"
            f"Catalyst Layer Analysis:\n"
            f"  R_CL: {R_CL:.4f} Ohm·cm² (using b_kin)"
        )
        ...
        self.ax.clear()
        self.ax.grid(True)
        self.ax.plot(i, self.y1, 'k--', linewidth=2, label="E_rev")
        self.ax.plot(i, self.y2, 'b-', linewidth=2, label="E_rev + η_kin")
        self.ax.plot(i, self.y3, 'g-', linewidth=2, label="E_rev + η_kin + η_ohm")
        self.ax.plot(i, self.y4, 'm-', linewidth=2, label="E_rev + η_kin + η_ohm + η_RCL")
        self.ax.plot(i, self.y5, 'r-', linewidth=2, label="E_rev + η_kin + η_ohm + η_RCL + η_res")
        self.ax.plot(i, self.y6, 'co', markersize=4, label="Original LSV")
        self.ax.set_xlabel("Current Density (A/cm²)", fontsize=24)
        self.ax.set_ylabel("Potential (V)", fontsize=24)
        self.ax.set_title("LSV Overpotential Analysis", fontsize=28)
